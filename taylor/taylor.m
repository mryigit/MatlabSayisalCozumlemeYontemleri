...
% Last Modified by GUIDE v2.5 19-Nov-2012 23:02:15
...
function fonksiyon_Callback(hObject, eventdata, handles)
    handles.fonk = inline( get(hObject,'String') );
guidata(hObject, handles);

function fonksiyon_CreateFcn(hObject, eventdata, handles)
...
function pushbutton1_Callback(hObject, eventdata, handles)
    gecmen = {};
    gecmen{1} = ['Derece : ',num2str(handles.der)];
    gecmen{2} = ' ';
    gecmen{3} = 'Sonuc';
    gecmen{4} = num2str( taylora(handles.fonk, handles.x0, handles.x1, handles.der),'%.10f');
    handles.goster = gecmen;
    set(handles.taygos,'String',handles.goster );
guidata(hObject, handles);


% --- Executes during object creation, after setting all properties.
function taygos_CreateFcn(hObject, eventdata, handles)



function derece_Callback(hObject, eventdata, handles)
    handles.der = str2double(get(hObject,'String'));
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function derece_CreateFcn(hObject, eventdata, handles)
% hObject    handle to derece (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
